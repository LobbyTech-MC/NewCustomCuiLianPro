...
public class CuiLianAPI {
    ...
    public static ItemStack setItemLevel(ItemStack item, Level level) {
        if (canCuiLian(item)) {
            ...
            for (i = 0; i < lore.size(); i++) {
            	...
            	String lineLore = lore.get(i);
            	if (lineLore != null && lineLore != "") {
            		Component parsed = mm.deserialize(lineLore);
                	lore.set(i, mm.serialize(parsed));
            	}
            }
    ...
    public static void setDisplayName(Material type, ItemMeta meta, int basicLevel) {
        if (NewCustomCuiLianPro.displayNameFormat == 1) {
            String displayName = meta.hasDisplayName() ? meta.getDisplayName() : LangUtils.getMaterialName(type);
            ...
            MiniMessage mm = MiniMessage.miniMessage();
            Component parsed = mm.deserialize(displayName);
            meta.setDisplayName(mm.serialize(parsed));
        }
    }
